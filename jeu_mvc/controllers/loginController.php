...
class loginController extends coreController{
	...
	public function validFormInscription(array $data){
		...
		if(!empty($_POST['subscribe'])){
			...
			if(!empty($error)){
				...
				if ($uManager->addUser($params)) {
					echo "Utilisateur <bold>".$params['login']."</bold> ajouté avec succès !";
	...
	// 		if($user = $this->getModel()->verifyUser($params = array('login'=>$login, 'mdp'=>$mdp))){
